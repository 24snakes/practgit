#scroll down and change the url ciapi.chatscrum.com to your ipaddress with the port that will be used for your backend server. e.g 1.2.3.4:5000
#also navigate to the directory below. /web/Angular/app/
#open the data.service.ts file.
#change the https to http and wss to ws.
#save your file and navigate back to where the dockerfile is located and build a new image. 
...
RUN yum -y install uwsgi-logger-file uwsgi-plugin-python36u --skip-broken
...
RUN cd /web/Chatscrum-Angular/ && sed -i 's/127.0.0.1:8000/34.70.47.62:5000/' src/app/data.service.ts;
